...
public class CameraControlTrigger : MonoBehaviour
{
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        if (collision.CompareTag("Player"))
        {
            ...
            {
                Debug.Log("CameraControlTrigger: OnTriggerEnter2D - swapping cameras");
                CameraManager.instance.SwapCamera(customInspectorObjects.cameraOnLeft, customInspectorObjects.cameraOnRight, exitDirection);
            }

            if (customInspectorObjects.panCameraOnContact)
            {
                Debug.Log("CameraControlTrigger: OnTriggerEnter2D - panning to offset");
                CameraManager.instance.PanCameraOnContact(customInspectorObjects.panDistance, customInspectorObjects.panTime, customInspectorObjects.panDirection, false);
            }
        }
    }

    private void OnTriggerExit2D(Collider2D collision) // <-- fixed name
    {
        if (collision.CompareTag("Player"))
        {
            if (customInspectorObjects.panCameraOnContact)
            {
                Debug.Log("CameraControlTrigger: OnTriggerExit2D - panning back to start");
